...
void InputWindow::paint(cairo_t *cr, unsigned int width, unsigned int height) {
    ...
    // 候选框右下角"设置"按钮：绿色空心圆环（首选输入的高亮绿），内部留空，
    // 放在最后一个候选右侧、间距与候选区间距一致（textMargin）、垂直居中、尺寸约等于一个汉字
    if (nCandidates_ && !candidateRegions_.empty()) {
        const auto &last = candidateRegions_.back();
        // 圆环尺寸略放大，便于提示为可点击按钮
        double sz = static_cast<double>(ConfigButtonSize) * 1.3;
        double centerY = (last.top() + last.bottom()) / 2.0;
        // 与候选区保持"候选区间距"相同的间距
        double gap = *textMargin.marginLeft + *textMargin.marginRight;
        double cfgLeft = last.right() + gap;
        double cfgRight = cfgLeft + sz;
        // 避免按钮超出候选框右缘
        double maxRight = width - *margin.marginRight;
        if (cfgRight > maxRight) {
            cfgRight = maxRight;
            cfgLeft = cfgRight - sz;
        }
        double cfgTop = centerY - sz / 2.0;
        if (cfgTop < 0) {
            cfgTop = 0;
        }
        configRegion_.setPosition(cfgLeft, cfgTop);
        configRegion_.setSize(sz, sz);

        cairo_save(cr);
        double cx = (cfgLeft + cfgRight) / 2.0;
        double cy = centerY;
        double r = sz * 0.40;
        const double tau = 6.283185307179586;
        // hover 时显示浅蓝圆角矩形背景（与候选高亮区域同款圆角样式），提示可点击设置
        if (configHovered_) {
            cairo_set_source_rgba(cr, 0.55, 0.75, 0.98, 0.72);
            double bx = cfgLeft - sz * 0.25;
            double by = cfgTop - sz * 0.25;
            double bw = sz * 1.5;
            double bh = sz * 1.5;
            double rad = bh * 0.30;
            cairo_new_path(cr);
            cairo_arc(cr, bx + rad, by + rad, rad, M_PI, 1.5 * M_PI);
            cairo_arc(cr, bx + bw - rad, by + rad, rad, 1.5 * M_PI, 2 * M_PI);
            cairo_arc(cr, bx + bw - rad, by + bh - rad, rad, 0, 0.5 * M_PI);
            cairo_arc(cr, bx + rad, by + bh - rad, rad, 0.5 * M_PI, M_PI);
            cairo_close_path(cr);
            cairo_fill(cr);
        }
        // 绿色空心圆环（首选输入的高亮绿，内部留空）
        const Color &ringColor = parent_->theme().inputPanelHighlight();
        cairo_set_source_rgba(cr, ringColor.redF(), ringColor.greenF(),
                              ringColor.blueF(), 1.0);
        cairo_set_line_width(cr, std::max(2.0, sz * 0.13));
        cairo_set_line_cap(cr, CAIRO_LINE_CAP_ROUND);
        cairo_new_path(cr);
        cairo_arc(cr, cx, cy, r, 0, tau);
        cairo_stroke(cr);
        cairo_restore(cr);
    }
...
void InputWindow::refresh() {
    // 用当前保存的输入上下文重新 update，触发候选框重绘（主题切换后即时生效）。
    // 若保存的上下文已失效（nullptr），update 会隐藏窗口，属正常。
    update(inputContext_.get());
}

void InputWindow::openSettingsPanel() {
    // single-instance 由 run-panel.sh 内部的锁文件保证
    pid_t pid = fork();
    if (pid == 0) {
        ...
        // 通过 setsid 脱离会话，避免随候选框销毁
        setsid();
        std::string cmd = std::string("nohup '") + home +
                          "/fcitx5-wechat-panel/run-panel.sh' >/dev/null 2>&1 &";
        execl("/bin/sh", "sh", "-c", cmd.c_str(), (char *)nullptr);
        _exit(127);
    }
    // 父进程不 wait，避免阻塞；用 SIGCHLD 回收即可
}
...
bool InputWindow::hover(int x, int y) {
    ...
    if (!prevHovered) {
        ...
        if (!nextHovered) {
            configHovered = configRegion_.contains(x, y);
            if (!configHovered) {
                for (int idx = 0, e = candidateRegions_.size(); idx < e;
                     idx++) {
                    if (candidateRegions_[idx].contains(x, y)) {
                        hoverIndex_ = idx;
                        break;
    ...
    needRepaint = needRepaint || configHovered_ != configHovered;
    configHovered_ = configHovered;
...
void InputWindow::updateYogaLayout() {
    ...
    // Configure root node
    // 为右下角"设置"按钮预留右侧空间（按钮在最后一个候选右侧、间距同候选区间距）
    double rightPadding = *margin.marginRight;
    if (nCandidates_ > 0) {
        double gap = *textMargin.marginLeft + *textMargin.marginRight;
        // 为右下角"设置"按钮预留空间：左右各留一个候选间距，使圆环左右间隙对称
        rightPadding +=
            2 * gap + std::max<double>(ConfigButtonSize * 1.3, fontHeight);
    }
    YGNodeStyleSetPadding(rootNode_.get(), YGEdgeLeft, *margin.marginLeft);
    YGNodeStyleSetPadding(rootNode_.get(), YGEdgeRight, rightPadding);
