...
ClassicUI::ClassicUI(Instance *instance) : instance_(instance) {
    ...
    // 启动对 classicui.conf 的监听，切主题/字号即时生效
    watchThemeConfig();
...
void ClassicUI::watchThemeConfig() {
    // 必须是 PkgConfig（~/.config/fcitx5），而不是 Config（~/.config）：
    // 面板写的是 ~/.config/fcitx5/conf/classicui.conf，用错类型会 stat 到
    // 不存在的 ~/.config/conf/classicui.conf，导致监听永不触发。
    ...
    themeConfigWatchEvent_ = instance_->eventLoop().addTimeEvent(
        CLOCK_MONOTONIC, now(CLOCK_MONOTONIC) + 800000, 800000,
        [this, path, lastMtime](EventSourceTime *, uint64_t) mutable {
            struct stat cur;
            // 配置文件被外部修改（面板切主题/字号）时，自动重载并刷新候选框
            if (::stat(path.c_str(), &cur) == 0 && cur.st_mtime != lastMtime) {
                lastMtime = cur.st_mtime;
                reloadConfig();
            }
            return true;
        });
...
void ClassicUI::reloadTheme() {
    ...
    // 主题已重载，立即刷新所有 UI 的候选框以即时应用新主题
    for (auto &[name, ui] : uis_) {
        FCITX_UNUSED(name);
        ui->refreshTheme();
    }
}
